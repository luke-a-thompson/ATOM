...
class NIN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, C, H, W)
        y: torch.Tensor = torch.einsum("bchw,co->bohw", x, self.W) + self.b[None, :, None, None]
        if y.stride()[1] == 1:
            y = y.contiguous()
        return y
...
class SpectralConv1d(nn.Module):
    def __init__(self, in_ch, out_ch, modes1):
        ...
        self.weights1 = nn.Parameter(self.scale * torch.rand(in_ch, out_ch, self.modes1, 2, dtype=torch.float))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.amp.autocast(device_type="cuda", enabled=False):
            ...
            x_ft: torch.Tensor = torch.fft.rfftn(x.float(), dim=[0])
            # Multiply relevant Fourier modes
            out_ft: torch.Tensor = compl_mul1d(x_ft[: self.modes1], torch.view_as_complex(self.weights1))
            # Return to physical space
            x: torch.Tensor = torch.fft.irfftn(out_ft, s=[T], dim=[0])
        return x
...
@torch.jit.script
def compl_mul1d_x(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    # (M, N, in_ch), (in_ch, out_ch, M) -> (M, N, out_channel)
...
class SpectralConv1d_x(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.amp.autocast(device_type="cuda", enabled=False):
            ...
            x_ft: torch.Tensor = torch.fft.rfftn(x.float(), dim=[0])
            # Multiply relevant Fourier modes
            out_ft: torch.Tensor = compl_mul1d_x(x_ft[: self.modes1], torch.view_as_complex(self.weights1))
            # Return to physical space
            x_out: torch.Tensor = torch.fft.irfftn(out_ft, s=[T], dim=[0])
        return x_out


class TimeConv_x(nn.Module):
    def __init__(self, in_ch, out_ch, modes, act, with_nin: bool = False):
    ...
    @override
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # x: [T, N, D, C]
