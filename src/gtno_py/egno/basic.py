...
from typing import override, Literal, final
...
class BaseMLP(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, activation: nn.Module, residual: bool = False, last_act: bool = False, flat: bool = False):
        ...
        if last_act:
            self.mlp = nn.Sequential(nn.Linear(input_dim, hidden_dim), activation, nn.Linear(hidden_dim, output_dim), activation)
        else:
            self.mlp = nn.Sequential(nn.Linear(input_dim, hidden_dim), activation, nn.Linear(hidden_dim, output_dim))
...
class EquivariantScalarNet(nn.Module):
    def __init__(self, n_vector_input, hidden_dim, activation, n_scalar_input=0, norm=True, flat=True):
        ...
        self.norm = norm
        self.in_scalar_net = BaseMLP(self.input_dim, self.hidden_dim, self.hidden_dim, self.activation, last_act=True, flat=flat)
    ...
    @override
    def forward(self, vectors: torch.Tensor, scalars: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        scalar: torch.Tensor = self.in_scalar_net(scalar)  # [N, K]
        vec_scalar: torch.Tensor = self.out_vector_net(scalar)  # [N, K]
...
class InvariantScalarNet(nn.Module):
    def __init__(self, n_vector_input, hidden_dim, output_dim, activation, n_scalar_input=0, norm=True, last_act=False, flat=False):
        """
        ...
        self.activation = activation
        self.norm = norm
        self.scalar_net = BaseMLP(self.input_dim, self.hidden_dim, self.output_dim, self.activation, last_act=last_act, flat=flat)

    @override
    def forward(self, vectors: torch.Tensor, scalars=None):
        """
...
class EGNN_Layer(nn.Module):
    def __init__(self, in_edge_nf, hidden_nf, activation: nn.Module = nn.SiLU(), with_v: bool = False, flat: bool = False, norm: bool = False, h_update: bool = True):
        super(EGNN_Layer, self).__init__()
        self.with_v = with_v
        self.edge_message_net = InvariantScalarNet(
            n_vector_input=1, hidden_dim=hidden_nf, output_dim=hidden_nf, activation=activation, n_scalar_input=2 * hidden_nf + in_edge_nf, norm=norm, last_act=True, flat=flat
        )
        self.coord_net = BaseMLP(input_dim=hidden_nf, hidden_dim=hidden_nf, output_dim=1, activation=activation, flat=flat)
        if self.with_v:
            self.node_v_net = BaseMLP(input_dim=hidden_nf, hidden_dim=hidden_nf, output_dim=1, activation=activation, flat=flat)
        else:
            self.node_v_net = None
        self.h_update: bool = h_update
        if self.h_update:
            self.node_net = BaseMLP(input_dim=hidden_nf + hidden_nf, hidden_dim=hidden_nf, output_dim=hidden_nf, activation=activation, flat=flat)

    @override
    def forward(self, x: torch.Tensor, h: torch.Tensor, edge_index: torch.LongTensor, edge_fea: torch.Tensor, v: torch.Tensor | None = None):
        row, col = edge_index
...
class EGNN(nn.Module):
    ...
    @override
    def forward(
        self, x: torch.Tensor, h: torch.Tensor, edge_index: torch.LongTensor, edge_fea: torch.Tensor, v: torch.Tensor | None = None
...
class EGMN(nn.Module):
    def __init__(self, n_layers: int, n_vector_input, hidden_dim, n_scalar_input, activation=nn.SiLU(), norm=False, flat=False):
        super(EGMN, self).__init__()
        self.layers = nn.ModuleList()
        self.n_layers = n_layers
        for i in range(self.n_layers):
            cur_layer = EquivariantScalarNet(
                n_vector_input=n_vector_input + i, hidden_dim=hidden_dim, activation=activation, n_scalar_input=n_scalar_input if i == 0 else hidden_dim, norm=norm, flat=flat
            )
...
class GNN_Layer(nn.Module):
    def __init__(self, in_edge_nf, hidden_nf, activation=nn.SiLU(), with_v=False, flat=False):
        ...
        self.with_v = with_v
        self.edge_message_net = BaseMLP(input_dim=in_edge_nf + 2 * hidden_nf, hidden_dim=hidden_nf, output_dim=hidden_nf, activation=activation, flat=flat)
        self.node_net = BaseMLP(input_dim=hidden_nf + hidden_nf, hidden_dim=hidden_nf, output_dim=hidden_nf, activation=activation, flat=flat)

    @override
    def forward(self, h: torch.Tensor, edge_index: torch.LongTensor, edge_fea: torch.Tensor) -> torch.Tensor:
        row, col = edge_index
        hij = torch.cat((h[row], h[col], edge_fea), dim=-1)  # [BM, 2K+T]
        message: torch.Tensor = self.edge_message_net(hij)  # [BM, K]
...
class GNN(nn.Module):
    def __init__(self, n_layers, in_node_nf, in_edge_nf, hidden_nf, activation=nn.SiLU(), device="cpu", flat=False):
        ...
        for i in range(self.n_layers):
            ...
            self.layers.append(layer)
        self.decoder = nn.Sequential(nn.Linear(hidden_nf, hidden_nf), activation, nn.Linear(hidden_nf, 3))
        self.to(device)
...
class RF_vel(nn.Module):
    def __init__(self, hidden_nf, edge_attr_nf=0, device: Literal["cpu", "cuda"] = "cpu", act_fn: nn.Module = nn.SiLU(), n_layers=4):
...
class GCL_rf_vel(nn.Module):
    def __init__(self, nf=64, edge_attr_nf=0, act_fn=nn.LeakyReLU(0.2), coords_weight=1.0):
        ...
        _ = torch.nn.init.xavier_uniform_(layer.weight, gain=0.001)
...
class FullMLP(nn.ModuleList):
    def __init__(self, in_node_nf, hidden_nf, n_layers, activation: nn.Module = nn.SiLU(), flat=False, device: Literal["cpu", "cuda"] = "cpu"):
        super(FullMLP, self).__init__()
        self.layers = nn.ModuleList()
        self.embedding = nn.Linear(in_node_nf, hidden_nf)
        for i in range(n_layers):
            self.layers.append(BaseMLP(hidden_nf, hidden_nf, hidden_nf, activation, residual=True, last_act=True, flat=flat))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(len(self.layers)):
            x = self.layers[i](x)
        out: torch.Tensor = self.output(x)
        return out
