...
class StandardLift(nn.Module):
    ...
    @override
    def forward(
        self, x_0: torch.Tensor, v_0: torch.Tensor, concatenated_features: torch.Tensor
...
class QuasiEquivariantTPLift(nn.Module):
    def __init__(
        ...
        self.concat_feats_linear = o3.FullyConnectedTensorProduct(
            lifting_dim_irreps, lifting_dim_irreps, lifting_dim_irreps
        )
...
class CanonicalizationLift(nn.Module):
    ...
    def forward(
        ...
        sign_e1: torch.Tensor = torch.where(
            (e1 * c0).sum(dim=-1, keepdim=True) >= 0, 1.0, -1.0
        ).to(e1.dtype)
        e1 = e1 * sign_e1
        ...
        sign_e3: torch.Tensor = torch.where(
            (e3 * c0).sum(dim=-1, keepdim=True) >= 0, 1.0, -1.0
        ).to(e3.dtype)
        e2 = e2 * sign_e3
        e3 = e3 * sign_e3
        ...
        Q: torch.Tensor = Q0.unsqueeze(1).expand(
            -1, x_0.shape[1], -1, -1
        )  # [B, T, 3, 3]
        ...
        vz_can: torch.Tensor = torch.cat(
            (v_can, concatenated_features[..., -1:]), dim=-1
        )  # Include Z channel
        ...
        lifted_concat_features: torch.Tensor = self.concat_feats_linear(
            lifted_x_can, lifted_vz_can
        )
