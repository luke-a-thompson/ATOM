...
class EGNO(nn.Module):
    ...
    def forward(
        ...
        time_emb = self._timestep_embedding(T, self.time_embed_dim).to(
            batch["x_0"].device
        )
        time_emb = (
            time_emb.unsqueeze(1).repeat(1, B * N, 1).view(B * T * N, -1)
        )  # [B*T*N, H_t]
        ...
        loc_mean = (
            loc_mean_repeated_nodes.unsqueeze(1).repeat(1, T, 1, 1).view(B * T * N, 3)
        )  # [B*T*N, 3]
        ...
        if "nodes" in batch:
            ...
            h = torch.zeros(
                B * T * N, 2, device=batch["x_0"].device, dtype=batch["x_0"].dtype
            )
        ...
        loc_dist0 = torch.sum(
            (loc0_flat[rows0] - loc0_flat[cols0]) ** 2, dim=-1, keepdim=True
        )  # [B, E, 1]
        loc_dist = (
            loc_dist0.unsqueeze(1).expand(B, T, E, 1).reshape(B * T * E, 1)
        )  # [B*T*E, 1]
        ...
        edge_attr = (
            edge_attr_base.unsqueeze(1)
            .expand(B, T, E, edge_attr_base.shape[-1])
            .reshape(B * T * E, -1)
        )
        ...
        if "edge_mask" in batch:
            edge_mask_b = (
                batch["edge_mask"].unsqueeze(1).expand(B, T, E).reshape(B * T * E)
            )
        ...
        for i in range(self.num_layers):
            if self.use_time_conv:
                ...
                x = (
                    temp[..., 0].view(B * T * N, 3) + loc_mean
                )  # Shape [B*T*N, 3] matches
    ...
    def _timestep_embedding(
        self, num_timesteps: int, lifting_dim: int, max_positions: int = 10_000
    ) -> torch.Tensor:
